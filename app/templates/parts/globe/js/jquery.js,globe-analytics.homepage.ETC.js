...
s.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,doc,docx,pdf,xls,xlsx,epub,mobi,ibooks"
...
//Get AAM Segments
s.list1=s.getAamSegments('aam_sc','aamsc');
...
/* End Optimizely/SiteCatalyst Integration */
...
if (methode.freeviewCountIncremented) {
	s.prop48= methode.freeviewCount;
}

/*
   Read RDB cookie and get userid from tail end of cookie.
   Also, used in globe-adinclude.js
   Maybe clean this code up at some point?
   Read cookie in only one file.
*/
function checkRDBCookie() {
   var rdb = getRDBCookie('RDB');
   if (rdb != null && rdb != '') {
      //if the cookie exists, get our data
      return rdb.substring(76,86);
   } else {
      return "";
   }
}
function getRDBCookie(c_name) {
...
s.eVar12=checkRDBCookie();

function getDaysOnMeter() {
   var fm = globe.cookie.get('FM');
   if (!fm) return;
   var startDateString = fm.split(':')[0].replace(/\"/g, ''), // see BGLOBE-3644 for replace reason
       year = parseInt(startDateString.substr(0,4),10),
       month = parseInt(startDateString.substr(4,2),10) - 1;
       day = parseInt(startDateString.substr(6,2),10),
       startDate = new Date(year, month, day),
       today = new Date();
       oneDay = 1000*60*60*24,
       delta = today.getTime() - startDate.getTime(),
       daysOnMeter = Math.ceil(delta / oneDay) ;
      
  return 'Day ' + daysOnMeter;
  
}
s.eVar4=getDaysOnMeter();
/************************** PLUGINS SECTION *************************/
...
(function() {
var comscore_s = document.createElement("script"), el = document.getElementsByTagName("script")[0]; comscore_s.async = true;
comscore_s.src = (document.location.protocol == "https:" ? "https://sb" : "http://b") + ".scorecardresearch.com/beacon.js";
el.parentNode.insertBefore(comscore_s, el);
})();

//init Omniture
(function(){                                                                                                                                                                                                                                                                 
    s.rl = {}; // defensive
    $.extend( s, globe.analytics.omniture );
    // Begin Audience Manager Block
    // s.t(); happens after this block
	...
	DIL.modules.siteCatalyst.init(_scDilObj, bgDil, {
	...
	if(! objIsEmpty(uriData.uriParams)){ 
		bgDil.api.signals(uriData.uriParams, 'c_');
	};
	if(objIsEmpty(uriData.pathname)){ 
		delete uriData.pathname;
	};
	delete uriData.uriParams;
	bgDil.api.signals(DIL.tools.getMetaTags('keywords'), 'c_');
	bgDil.api.signals(uriData, 'c_').submit();
	
	if(bgDil.helpers.getCookie('s_vi'))
	{	
		var sID = bgDil.helpers.getCookie('s_vi');
	...
	//bgId
	if(typeof bgDil != 'undefined' && typeof s.eVar12 != 'undefined' && s.eVar12 /*&& s.eVar12 != <ADD NOT LOGGED IN VALUE HERE>*/)
	{
		bgDil.api.aamIdSync({
			dpid: '1292',
			dpuuid: s.eVar12,
    ...
    // End Audience Manager block
    // console.log(s);
	// The actual Omniture init call                                                                                                                                                                                                                                                                                                                                                                                                                                                                     
    s.t()                                                                                                                                                                                                                                                 
...
(function ($, undefined) {
	...
	$(document).ready(function(){
	    // Scoreboard switcher - switches the scoreboards between the sports
	    var $scores = $(".bg-scores .live-scores");
	    if ($scores.length > 1) {
	        // Build navigation dynamically (TK could/should this be static?)
	        var $stubScoreNav = $(innerShiv('<div class="score-nav"><nav><h1></h1><ul></ul></nav></div>', false)).appendTo('.bg-scores'),
	        ...
	        for (var i = 0, lScores = $scores.length; i < lScores; i++) {
	            ...
	            if (i === 0) {
	                ...
	                $("#" + sSlug).addClass(sScoreClass.show);
	            }
	        ...
	        $scoreTabs.click(function (e) {
	            ...
	            $scores.filter(":not(#" + sSlug + ")").removeClass(sScoreClass.show);
	            $("#" + sSlug).addClass(sScoreClass.show);
    ...
    $(function () {
        var t = setInterval(getScores, 60000);
        //var t = setInterval(getScores, 3000); //For testing
        // Grab scores and update the board(s)
        function getScores() {
            $.ajax({
                url: "/_ajax/scores/json.jpt?_="+(''+Math.random()).substring(2,4),
                //url: "/_ajax/scores/test_scores.json",
                dataType: "json",
                success: function (data) {
                    var scores = data.scores;
                    if (scores.mlb && $(".score-redsox")) updateScore(scores.mlb, "redsox");
                    if (scores.nba && $(".score-celtics")) updateScore(scores.nba, "celtics");
                    if (scores.nfl && $(".score-pats")) updateScore(scores.nfl, "pats");
                    if (scores.nhl && $(".score-bruins")) updateScore(scores.nhl, "bruins");
                }
            })
        }
        // Number to ordinal 1 -> "1st", etc
        function toOrdinal(num) {
            num = parseInt(num);
            var rem = num % 10;
            return num + (rem == 1 ? 'st' : rem == 2 ? 'nd' : rem == 3 ? 'rd' : 'th');
        }
        // Renders the sport's period/quater/inning(&top/bot)
        function toSportPeriod(team, num, topOrBot) {
            if (team == 'redsox') {
                return '<span class="bottom-inning">' + (topOrBot.toLowerCase() == 'top' ? '&#9650;' : '&#9660;') + '&nbsp;</span>' + toOrdinal(num) + ' Inning';
            } else if (team == 'celtics') {
                if (num > 4) {
                    return 'OT';
                } else {
                    return toOrdinal(num) + ' Quarter';
                }
            } else if (team == 'bruins') {
                if (num > 3) {
                    if (num > 4) return 'Shootout';
                    return 'OT';
                } else {
                    return toOrdinal(num) + ' Prd';
                }
            } else if (team == 'pats') {
                if (num > 4) {
                    return 'OT';
                } else {
                    return toOrdinal(num) + ' Qtr';
                }
            }
        }
        // Renders the current game time, or not, for baseball
        function toSportTime(team, game) {
            if (team == "redsox"){
				return '<span class="play">' + game.outs + ' outs</span>';
			}
            game.seconds = Math.floor(game.seconds);
            if (game.seconds < 10) game.seconds = "0" + game.seconds;
            return '<span class="time">' + game.minutes + ':' + game.seconds + '</span>';
        }
        // Render's the current football play, down, distance, yards to go
        // Could be expanded with more data from the original STATS feed...
        function getPlay(team, game) {
            if ((team != 'pats') || (game.down == '') || (game.distance == '')) return '';
            var yards = parseInt(game.yards_from_goal),
                placement = "Opp's ";
            if (yards >= 50) {
                placement = "Own ";
                yards = 100 - yards;
            }
            return '<span class="play">' + toOrdinal(game.down) + ' &#38; ' + game.distance + ', ' + placement + yards + '</span>';
        }
		

		// gets the bases ONLY for baseball
		function getBases(team, game){
			var firstbase = game.firstb;
			var secondbase = game.secondb;
			var thirdbase = game.thirdb;
			
			if((firstbase != "0") && (secondbase != "0") && (thirdbase != "0")){
				return "loaded";
			} else if((firstbase != "0") && (secondbase == "0") && (thirdbase == "0")) {
				return "first";
			} else if((firstbase == "0") && (secondbase != "0") && (thirdbase == "0")) {
				return "second";
			} else if((firstbase == "0") && (secondbase == "0") && (thirdbase != "0")) {
        ...
        // Does a full update of one sport's scoreboard
        function updateScore(game, team) {
            // Grab current home and away score
            var homeScore = parseInt($("#score-" + team + " .score-second .score").html()),
                awayScore = parseInt($("#score-" + team + " .score-first .score").html());
            // Check and change scores
            if (homeScore != game.homeScore) {
                // Change to roll effect
                $("#score-" + team + " .score-second .score").hide().html(game.homeScore).fadeIn();
            }
            if (awayScore != game.awayScore) {
                // Change to roll effect
                $("#score-" + team + " .score-first .score").hide().html(game.awayScore).fadeIn();
            }
            if (game.awayPos == true) {
                if (!($('#score-' + team + ' .score-first .possession').length)) {
                    var posSpan = '<span class="possession" title="Has possession">(P)</span>';
                    $(posSpan).prependTo($('#score-' + team + ' .score-first'));
                }
            } else {
                $('#score-' + team + ' .score-first .possession').remove()
            }
            if (game.homePos == true) {
                if (!($('#score-' + team + ' .score-second .possession').length)) {
                    var posSpan = '<span class="possession" title="Has possession">(P)</span>';
                    $(posSpan).prependTo($('#score-' + team + ' .score-second'));
                }
            } else {
                $('#score-' + team + ' .score-second .possession').remove()
            }
            // Update other info
            var score_stats = $("#score-" + team + " .score-stats");
            if (game.status == "Pre-Game") {
                if (score_stats.html() != "Game starts at " + game.sTime) {
                    score_stats.hide().html("Game starts at " + game.sTime).fadeIn();
                }
            } else if (game.status == "Final") {
                if (score_stats.html() != "Final") {
                    score_stats.hide().html("Final").fadeIn()
                }
            } else if (game.status == "In-Progress") {
                if((team == "bruins") && (game.segment_number == "5")){
                	score_stats.hide().html("Shootout").fadeIn();
                } else if((parseFloat(game.minutes) == 0) && (parseFloat(game.seconds) == 0)) {
                    if ((team == "celtics") || (team == "pats")) {
                        if ((game.segment_number == "1") || (game.segment_number == "3")) {
                            score_stats.hide().html("End of " + toOrdinal(game.segment_number) + " Qtr").fadeIn();
                        } else if (game.segment_number == "2") {
                            score_stats.hide().html("Halftime").fadeIn();
                        } else if (game.segment_number == "4") {
                            score_stats.hide().html("End of Regulation").fadeIn();
                        } else if (parseFloat(game.segment_number) > 4) {
                            if (team == "pats") {
                                score_stats.hide().html("End of Overtime").fadeIn();
                            } else {
                                var otNum = parseFloat(game.segment_division) - 4;
                                score_stats.hide().html("End of " + toOrdinal(otNum) + " Overtime").fadeIn();
                            }
                        } else {
							score_stats.hide().html("End of Game").fadeIn();
						}
                    } else if (team == "bruins") {
                        if (game.segment_number == "3") {
                            score_stats.hide().html("End of Regulation").fadeIn();
                        } else if ((game.segment_number == "1") || (game.segment_number == "2")) {
                            score_stats.hide().html(toOrdinal(game.segment_number) + " Intermission").fadeIn();
                        } else if (game.segment_number == "4") {
                            score_stats.hide().html("End of Overtime").fadeIn();
                        } else if (game.segment_number == "5") {
                            score_stats.hide().html("End of Shootout").fadeIn();
                        } else {
							score_stats.hide().html("End of Game").fadeIn();
						}
                    }
                } else {
                    var periodSpan = toSportPeriod(team, game.segment_number, game.segment_division),
                        periodSpan = '<span class="period">' + periodSpan + '&nbsp;</span>',
                        timeSpan = toSportTime(team, game),
                        playSpan = getPlay(team, game);
                    score_stats.hide().html(periodSpan + timeSpan + playSpan).fadeIn();
					
					if(team == "redsox" && game.status == "In-Progress"){
						$(".bases").hide().attr("class", "bases").addClass("" + getBases(team, game)).fadeIn();
					}
                }
            }
        }
...
(function (win, undefined) {
  ...
  Array of ad positions, and corresponding widths/heights.
    ...
    * [].DFPPosition / [].pos / [].width / [].height are required for all active positions

  We suggest careful care and maintenance of this array. Prune often! :)

  */

  globe.dfp.adCatalog = [
  {
    DFPPosition: "ad_bigbox2",
    pos: "btf",
    width: "300",
    height: "250"
  },
  {
    DFPPosition: "ad_bigbox1",
    pos: "atf",
    width: "300",
    height: "250"
  },
  {
      DFPPosition: "ad_outofpage2",
      pos: "billboard",
      width: "300",
      height: "250"
  }
      // {
  //   DFPPosition: "ad_sticky",
  //   pos: "sticky",
  //   height: "85",
  //   width: "180"
  // },
...
(function(win, $, undefined){
    ...
    // Build lookup from the globe.dfp.adCatalog table
    var adKeys = [];
    for (var i in globe.dfp.adCatalog) {
        var obj = globe.dfp.adCatalog[i];

        adKeys[obj.DFPPosition] = {};
        adKeys[obj.DFPPosition].pos = obj.pos || false;
        adKeys[obj.DFPPosition].width = obj.width || "100%";
        adKeys[obj.DFPPosition].height = obj.height || "250";
    }

    //this can be called to initialize ad divs that do not yet have ads appended
    //call on DomReady
    globe.dfp.loader = function() {
        if ( !globe.dfp.adUnit ) { return; }
        
        // Strip www. from adUnit / This feels wrong
        var www = globe.dfp.adUnit.indexOf('www.');
        if ( www === 0 ) { globe.dfp.adUnit = globe.dfp.adUnit.replace('www.', ''); }
        //grabs all ads with class of 'ad' with data-adname attribue
        var $ads = $( ".ad:not([data-adname-complete])" );
        var iAds = $ads.length;
        var networkCode = 61381659;
        var activeAds = [];

      // Initial loop to set up the ads / remove unused DFP slots
        for ( var i = 0; i < iAds; i++ ) {

            var $thisAd = $( $ads[i] );
            var DFPPosition = $thisAd.attr("id");

            // Is there a registered ad with this position name?
            if ( adKeys[DFPPosition] ) {

                // Is there a corresponding DFP position?
               var adWidth = parseInt(adKeys[DFPPosition].width, 10);
               var adHeight = parseInt(adKeys[DFPPosition].height, 10);
               //dfp specific; pushing commands into array
               googletag.cmd.push(function() {
                  googletag.defineSlot("/" + networkCode + "/" + globe.dfp.adUnit , [adWidth,adHeight],  DFPPosition)
                     .addService(googletag.pubads())
                     .setTargeting("pos", adKeys[DFPPosition].pos);
               });
      ...
      googletag.cmd.push(function() {
         ...
         //we are now ready to display ads
         googletag.enableServices();
      });

      // Check test query string
      globe.dfp.keyValuePairs.test = getQueryStringValue('test');

      // Loop throught globe.dfp.keyValuePairs object to add targeting
      var kvPairs = globe.dfp.keyValuePairs;
      ...
      // Loop to display the ads
      for ( var i = 0; i < activeAds.length; i++ ) {
         googletag.cmd.push(function() {
            googletag.display(activeAds[i]);
         });
      }
    ...
    /*
       Read RDB cookie and add appropriate values to globe.dfp.keyValuePairs
    ...
    */
    ...
    function cookieParser(cookie) {
       ...
       if (!$.isEmptyObject(obj)) {
          globe.dfp.keyValuePairs.age = obj.age;
          globe.dfp.keyValuePairs.zip = obj.zip5;
          globe.dfp.keyValuePairs.g = obj.gender;
       };
       return obj;
    }
    ...
    function addAAMValuesToKeyValuePairs(){
      var aam_dfp   = globe.cookie.get("aam_dfp"),          
          aam       = aam_dfp ? decodeURIComponent(aam_dfp).split(',')[0] : '',
          aam_value = aam ? aam.split('=')[1]: '';
      globe.dfp.keyValuePairs.aam = aam_value;
      globe.dfp.keyValuePairs.aam_uuid = globe.cookie.get("aam_uuid");
    }

    // Check RDB cookie
    checkCookie();
    addAAMValuesToKeyValuePairs();
    
    //run dfp loader on domready    
    $( globe.dfp.loader );
