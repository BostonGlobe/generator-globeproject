...
(function (win, undefined) {
  ...
  Array of ad positions, and corresponding widths/heights.
    ...
    * [].OASPosition / [].DFPPosition / [].pos / [].width / [].height are required for all active positions
    * That is until after the OAS specific markup has been removed from all templates

  We suggest careful care and maintenance of this array (once OAS specific markup has been removed). Prune often! :)
  ...
  */
  ...
  {
    OASPosition: "SPONSOR1",
    height: "85",
    width: "180"
  },
  {
    OASPosition: "SPONSOR2",
    height: "150"
  },
  {
    OASPosition: "SPONSOR3",
    height: "150"
  },
  {
    OASPosition: "SPONSOR",
...
/* 
    ...
    * Currently maps OAS positions to corresponding DFP position
    * Ignores OAS positions that do not have a corresponding DFP position
    * References to OAS should be removed once the ad template markup has been updated
    * adShift() has not been touched
...
(function(win, $, undefined){
    ...
    for (var i in globe.OAS.adCatalog) {
        ...
        adKeys[obj.OASPosition] = {};
        adKeys[obj.OASPosition].DFP = obj.DFPPosition || false;
        adKeys[obj.OASPosition].pos = obj.pos || false;
        adKeys[obj.OASPosition].width = obj.width || "100%";
        adKeys[obj.OASPosition].height = obj.height || "250";
    }

    //this can be called to initialize ad divs that do not yet have ads appended
    //call on DomReady
    globe.OAS.loader = function() {
        if ( !globe.OAS.adUnit ) { return; }
        
        // Strip www. from adUnit / This feels wrong
        var www = globe.OAS.adUnit.indexOf('www.');
        if ( www === 0 ) { globe.OAS.adUnit = globe.OAS.adUnit.replace('www.', ''); }
      ...
      // Initial loop to set up the ads / remove unused OAS slots
        for ( var i = 0; i < iAds; i++ ) {

            var $thisAd = $( $ads[i] );
            var OASPosition = $thisAd.attr("data-adname");

            // Is there a registered ad with this position name?
            if ( adKeys[OASPosition] ) {

                // Is there a corresponding DFP position?
                if ( adKeys[OASPosition].DFP ) {
               var adWidth = parseInt(adKeys[OASPosition].width, 10);
               var adHeight = parseInt(adKeys[OASPosition].height, 10);
               googletag.cmd.push(function() {
                  googletag.defineSlot("/" + networkCode + "/" + globe.OAS.adUnit , [adWidth,adHeight],  adKeys[OASPosition].DFP)
                     .addService(googletag.pubads())
                     .setTargeting("pos", adKeys[OASPosition].pos);
               });

                    // Set attr to prevent re-running on this elem
                    $thisAd.attr({ id: adKeys[OASPosition].DFP, "data-adname-complete": true });

                    // Always load ads in the drawer. Otherwise, use the visibility check
                    if ( $thisAd.is( ":visible" ) || $thisAd.parents( ".saved-panel" ).length ) {
                  activeAds.push(adKeys[OASPosition].DFP);
                    } else {
                    ...
                    // Set attr to prevent re-running on this elem
                    // Remove the class applying the min-height: 265px rule
                    // This is OK because they should never have content, thus should never have a real height
                    $thisAd.attr( "data-adname-complete", true ).removeClass('ad');;
                }
            }
        }
      ...
      globe.OAS.keyValuePairs.test = getQueryStringValue('test');

      // Loop throught globe.OAS.keyValuePairs object to add targeting
      var kvPairs = globe.OAS.keyValuePairs;
    ...
    if ( globe.OAS.adUnit ) {
        var adShift = function() {
            ...
            var $ads = $( ".ad[data-adname]:not(.saved-ad)" );
            var iAds        = $ads.length;

            for ( var i = 0; i < iAds; i++ ) {
                var $thisAd     = $( $ads[i] );
                var sPos        = $thisAd.data("adname");
    ...
    */
    ...
    function cookieParser(cookie) {
       ...
       if (!$.isEmptyObject(obj)) {
          globe.OAS.keyValuePairs.age = obj.age;
          globe.OAS.keyValuePairs.zip = obj.zip5;
          globe.OAS.keyValuePairs.g = obj.gender;
       };
       return obj;
    }
